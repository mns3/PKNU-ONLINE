...
캠퍼스 규모의 공간을 사람 손으로 모델링하기엔 일정과 인력이 부족하였습니다. 그래서 저희는 AI를 이용한 자동화된 워크플로우를 제작하는 것을 1순위로 접근했습니다. GIS 데이터로 정확한 화이트박스 모델을 확보하고, 현장 사진을 입력으로 생성형 AI가 텍스처와 모델을 제작하고, 반복 오브젝트는 PCG 알고리즘이 배치하는 구조입니다.

제작된 가상현실 속 부경대학교는 다양한 사람들과 만나고 소통할 수 있는 공간, 폭우로 인한 침수등의 재난 시뮬레이션, 자율주행의 학습등 다양한 곳에 사용할 수 있는 다목적 공간입니다.
...
V-World에서 취득한 **건물 도면 데이터** 과 **건물 높이 DB** 를 결합해 화이트박스 모델을 생성했습니다.  이후 AI가 만드는 텍스처를 그 위에 얹히는 방식으로 중요도가 낮은 건물을 제작하였습니다.
...
현장에서 촬영한 사진을 이용하여 생성형 AI(Nano Banana)를 통해 그림자를 지우고, 기울기를 맞추는 등의 후보정을 하였습니다.
프롬포트 만으로 제작하는 생성형 AI의 결과는 편차가 커서, 품질이 운에 의존하게 됩니다. 저희는 퀄리티의 편차를 줄이기 위해 직접 촬영한 이미지를 후보정하는 과정을 넣었습니다.
