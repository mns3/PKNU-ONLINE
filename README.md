...
캠퍼스 규모의 공간을 사람 손으로 모델링하기엔 일정과 인력이 부족하였습니다. 그래서 저희는 AI를 이용한 자동화된 워크플로우를 제작하는 것을 1순위로 접근했습니다.
GIS 데이터로 정확한 화이트박스 모델을 확보하고, 현장 사진을 입력으로 생성형 AI가 텍스처와 모델을 제작하고, 반복 오브젝트는 PCG 알고리즘이 배치하는 구조입니다.
...
프롬포트 만으로 제작하는 생성형 AI의 결과는 편차가 커서, 품질이 운에 의존하게 됩니다.
저희는 퀄리티의 편차를 줄이기 위해 직접 촬영한 이미지를 후보정하는 과정을 넣었습니다.
